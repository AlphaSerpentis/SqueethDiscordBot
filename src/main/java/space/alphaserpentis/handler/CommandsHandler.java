...
import space.alphaserpentis.commands.*;
...
//    private static final ArrayList<String[]> listOfCommands = (ArrayList<String[]>) List.of(new String[]{"Ping", "Pong"});
    ...
    public static void checkAndSetSlashCommands() {
        ...
        for (Iterator<Command> it = listOfActiveCommands.iterator(); it.hasNext(); ) {
            Command cmd = it.next();
            if(mappingOfCommands.keySet().contains(cmd.getName())) {
